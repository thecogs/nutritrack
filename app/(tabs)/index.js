...
const MACRO_DEFS = [
  ...
  { key: 'fat',     label: 'Fat (Total)', unit: 'g', color: '#FFD700' },
  { key: 'fiber',   label: 'Fiber',       unit: 'g', color: '#215CDA' },
  { key: 'sugar',   label: 'Sugar',       unit: 'g', color: '#FF6B9D' },
  { key: 'sat_fat', label: 'Sat. Fat',    unit: 'g', color: '#FF6347' },
];
...
function AddFoodModal({ visible, onClose, onSave }) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      getGoals().then((g) => { userAllergensRef.current = g.allergens || []; }).catch(() => {});
    }
  }, [visible]);
  useEffect(() => {
    if (!base) return;
    ...
    setMacros({
      ...
      sat_fat: String(Math.round((base.sat_fat ||0) * f * 10) / 10),
    });
  }, [base, servingGrams]);
  ...
  const handleSave = async () => {
    ...
    await onSave({ food_name: name, calories: parseFloat(macros.calories)||0, protein: parseFloat(macros.protein)||0, carbs: parseFloat(macros.carbs)||0, fat: parseFloat(macros.fat)||0, fiber: parseFloat(macros.fiber)||0, sugar: parseFloat(macros.sugar)||0, sat_fat: parseFloat(macros.sat_fat)||0, meal_type: mealType });
    reset(); onClose();
  };
  ...
  const handleSaveMany = async () => {
    ...
    await onSave(selected.map((f) => ({ food_name: f.food_name, calories: f.calories||0, protein: f.protein||0, carbs: f.carbs||0, fat: f.fat||0, fiber: f.fiber||0, sugar: f.sugar||0, sat_fat: f.sat_fat||0, meal_type: mealType })));
    reset(); onClose();
  };
...
function EditFoodModal({ visible, item, onClose, onSave }) {
  ...
  useEffect(() => {
    if (item) {
      ...
      setMacros({ calories: String(item.calories||0), protein: String(item.protein||0), carbs: String(item.carbs||0), fat: String(item.fat||0), fiber: String(item.fiber||0), sugar: String(item.sugar||0), sat_fat: String(item.sat_fat||0) });
    }
  }, [item]);

  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('Name required'); return; }
    await onSave(item.id, { food_name: name.trim(), calories: parseFloat(macros.calories)||0, protein: parseFloat(macros.protein)||0, carbs: parseFloat(macros.carbs)||0, fat: parseFloat(macros.fat)||0, fiber: parseFloat(macros.fiber)||0, sugar: parseFloat(macros.sugar)||0, sat_fat: parseFloat(macros.sat_fat)||0, meal_type: mealType });
    onClose();
  };
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardDismissMode="on-drag">
        ...
        <View style={styles.heroCard}>
          ...
          <View style={styles.macroPillRow}>
            ...
            <MacroPill label="Sat. Fat" current={totals.sat_fat||0} goal={goals.sat_fat??20} color="#FF6347" />
          </View>
        </View>
