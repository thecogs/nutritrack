...
const SERVING_PRESETS = [
  { label: '1 oz', grams: 28 }, { label: '2 oz', grams: 57 },
  { label: '3 oz', grams: 85 }, { label: '4 oz', grams: 113 },
  { label: '6 oz', grams: 170 }, { label: '8 oz', grams: 227 },
  { label: '100g', grams: 100 }, { label: '150g', grams: 150 },
  { label: '200g', grams: 200 }, { label: '300g', grams: 300 },
];
...
function MealSection({ title, items, onDelete }) {
...
function TimelineView({ logs, onDelete }) {
    ...
    <View>
      {sorted.map((item, i) => (
        <View key={item.id} style={styles.timelineRow}>
          ...
          <View style={[styles.logItem, { flex: 1, marginBottom: 10 }]}>
            <View style={styles.logInfo}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={styles.logName}>{item.food_name}</Text>
                <Text style={styles.timelineBadge}>{item.meal_type}</Text>
              </View>
              <Text style={styles.logMacros}>
                {Math.round(item.calories)} kcal · P {item.protein}g · C {item.carbs}g · F {item.fat}g
              </Text>
            </View>
            <TouchableOpacity onPress={() => onDelete(item.id)} style={styles.deleteBtn}>
              <Text style={styles.deleteBtnText}>✕</Text>
...
function AddFoodModal({ visible, onClose, onSave }) {
  ...
  // null = single-food mode; array = multi-food review mode
  ...
  // Refresh time-based meal default each time the modal opens
  useEffect(() => {
    if (visible) setMealType(getDefaultMealType());
  }, [visible]);
  ...
  const handleTextChange = (t) => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        const data = await searchFood(t.trim());
        if (!pickedRef.current) setResults(data || []);
      } catch { setResults([]); }
  ...
  const handleEstimate = async () => {
    ...
    try {
      ...
      if (foods.length === 1) {
        // Single item — populate macro cells as before
        const food = foods[0];
        setText(food.food_name || text.trim());
        setBase(null);
        setMacros({
        ...
        setAiItems(foods.map((f) => ({ ...f, excluded: false })));
      }
    } catch {
      Alert.alert('AI estimate failed', 'Enter values manually below.');
    } finally {
      setEstimating(false);
    }
  };
  ...
  const toggleAiItem = (idx) => {
    setAiItems((prev) => prev.map((item, i) => i === idx ? { ...item, excluded: !item.excluded } : item));
  };
  ...
  const reset = () => {
    ...
    setMealType(getDefaultMealType()); setBase(null); setServingGrams(100);
    pickedRef.current = false;
  };

  const selectedItems = (aiItems || []).filter((i) => !i.excluded);
  const aiTotals = selectedItems.reduce(
    (acc, f) => ({
      ...
      <View style={styles.modalBg}>
        ...
        >
          ...
          {aiItems ? (
            // ── Multi-food review ──────────────────────────────────────────────
            <>
              <Text style={styles.aiMealHeader}>
                {selectedItems.length} food{selectedItems.length !== 1 ? 's' : ''} identified — tap ✕ to remove any
              </Text>

              {aiItems.map((item, idx) => (
                <View key={idx} style={[styles.aiItemCard, item.excluded && styles.aiItemCardExcluded]}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.aiItemName, item.excluded && styles.aiItemNameDim]} numberOfLines={2}>
                      {item.food_name}
                    </Text>
                    <Text style={styles.aiItemMacros}>
                      {Math.round(item.calories)} kcal · P {item.protein}g · C {item.carbs}g · F {item.fat}g
                    </Text>
                  </View>
                  <TouchableOpacity onPress={() => toggleAiItem(idx)} style={styles.aiItemToggle}>
                    <Text style={[styles.aiItemToggleText, item.excluded && styles.aiItemAddText]}>
                      {item.excluded ? 'Add' : '✕'}
                    </Text>
                  </TouchableOpacity>
                </View>
              ...
              {selectedItems.length > 1 && (
                <View style={styles.aiTotalRow}>
                  ...
                  <Text style={styles.aiTotalMacros}>
                    {Math.round(aiTotals.calories)} kcal · P {Math.round(aiTotals.protein)}g · C {Math.round(aiTotals.carbs)}g · F {Math.round(aiTotals.fat)}g
                  </Text>
                </View>
              )}
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity style={styles.cancelBtn} onPress={() => setAiItems(null)}>
                  <Text style={styles.cancelText}>Back</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.logBtn, !selectedItems.length && { backgroundColor: SURF }]}
                  onPress={handleSaveMany}
                  disabled={!selectedItems.length}
                >
                  <Text style={styles.logBtnText}>
                    Log {selectedItems.length} Food{selectedItems.length !== 1 ? 's' : ''}
                  </Text>
                </TouchableOpacity>
              </View>
            </>
            ...
            <>
              <View style={styles.inputRow}>
                <TextInput
                  ...
                  placeholder="Search or describe what you ate…"
              ...
              <TouchableOpacity
                style={[styles.estimateBtn, (!text.trim() || estimating) && styles.estimateBtnDisabled]}
                ...
                <>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.ozScroll} contentContainerStyle={{ paddingRight: 16 }}>
                    {SERVING_PRESETS.map((p) => (
                      <TouchableOpacity
                        key={p.label}
                        style={[styles.ozChip, servingGrams === p.grams && styles.ozChipActive]}
                        onPress={() => setServingGrams(p.grams)}
                      >
                        <Text style={[styles.ozChipText, servingGrams === p.grams && styles.ozChipTextActive]}>{p.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                </>
              ...
              <Text style={styles.macroNote}>
                {base ? 'Auto-scaled · tap to adjust' : estimating ? 'Estimating…' : 'Enter values manually or use AI / search above'}
              </Text>
              ...
              <View style={styles.modalActions}>
                ...
                <TouchableOpacity style={styles.logBtn} onPress={handleSave}>
...
export default function LogScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      async function load() {
        try {
          const [l, g] = await Promise.all([getTodayLogs(), getGoals()]);
          setLogs(l || []);
          if (g) setGoals(g);
  ...
  return (
    <>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardDismissMode="on-drag">
        ...
        {logs.length === 0 ? (
          ...
          <>
            <View style={styles.viewToggle}>
              {['meals', 'timeline'].map((v) => (
                <TouchableOpacity key={v} style={[styles.viewToggleBtn, view === v && styles.viewToggleBtnActive]} onPress={() => setView(v)}>
                  <Text style={[styles.viewToggleText, view === v && styles.viewToggleTextActive]}>
                    {v === 'meals' ? 'By Meal' : 'Timeline'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            {view === 'meals'
              ? MEAL_TYPES.map((type) => (
                  <MealSection key={type} title={type[0].toUpperCase() + type.slice(1)} items={grouped[type]} onDelete={handleDelete} />
                ))
              : <TimelineView logs={logs} onDelete={handleDelete} />
            }
          </>
        )}
      </ScrollView>
      ...
      <AddFoodModal visible={modalVisible} onClose={() => setModalVisible(false)} onSave={handleAddFood} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  heroRing: {
    ...
    borderWidth: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: BG,
  },
  ...
  deleteBtn:    { width: 28, height: 28, borderRadius: 14, backgroundColor: SURF, alignItems: 'center', justifyContent: 'center' },
  ...
  timelineBadge: { color: '#2E3040', fontSize: 10, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
